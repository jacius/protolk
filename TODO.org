...
- [ ] Prototypal inheritance is a fundamental characteristic of pobs,
...
- [ ] The abilities to resolve props and methods are inherent to all
  pobs, so these methods should be /required/ to be contained in every
  pob itself. These methods should be copied from the base pob when
  the pob is derived, rather than inherited.
